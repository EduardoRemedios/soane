# Pack Manifest

## Version

v1

## Change Log

- v1 (2026-07-05): Initial Stage J manifest.

## Run Metadata

- RUN_ID: RUN_20260705_0923_live_coding_adapter_eval_plan
- Sprint ID: LCAE-V0-001
- Created: 2026-07-05 09:23 local
- Execution Mode: PLANNING_ONLY
- Spec Versions: STAGE_CONTRACTS v4.14, PURPLE_GATE_CHECKLIST v3.3

## Required Run Root Files

- raw_brief.md: present
- KNOWLEDGE_LINT.txt: present
- CONTEXT_RECALL_REPORT.md: present
- EXECUTION_MODE.txt: present
- SPRINT_ID.txt: present

## Required Pack Files

- intent.md: present
- intent_redteam.md: present
- intent_synthesis.md: present
- intent_lock_report.md: present
- premortem.md: present
- risk_register.md: present
- verification_plan.md: present
- traceability_matrix.md: present
- micro_sprints.md: present
- LCAE-V0-001_ENVELOPE.md: present
- LCAE-V0-001_ENVELOPE_REDTEAM.md: present
- external_source_review.md: present
- fixtures/: present
- PACK_CHECKLIST.md: present
- PACK_MANIFEST.md: present
- PACK_AUDIT_REPORT.md: present

## Handoffs

- HANDOFF_STAGE_A.md: present
- HANDOFF_STAGE_B.md: present
- HANDOFF_STAGE_C.md: present
- HANDOFF_STAGE_D.md: present
- HANDOFF_STAGE_E.md: present
- HANDOFF_STAGE_F.md: present
- HANDOFF_STAGE_G.md: present
- HANDOFF_STAGE_H.md: present
- HANDOFF_STAGE_I.md: present
- HANDOFF_STAGE_J.md: present
- HANDOFF_STAGE_I2.md: present

## Verification Manifest

- verification_manifest.yaml present: no
- Reason: planning-only pack; runnable manifest belongs to future execution-enabled implementation if needed.
